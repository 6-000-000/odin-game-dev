...
default_settings :: proc() -> Settings {
	return {
		...
		avoid_radius = 35,
		w_sep = 1.4, // separation slightly strongest — personal space matters
		w_align = 1.0,
		w_coh = 1.0,
	}
}

// --- vector helpers (identical to lesson 8.1) ---
...
// --- the full three rules, the naïve way: EVERY boid checks EVERY boid ---
...
update_boids :: proc(boids: []Boid, s: Settings, dt: f32) {
	for &b, i in boids {
		...
		// steer: each rule becomes a clamped "desired − current" velocity
		accel: rl.Vector2
		if perc_count > 0 {
		...
		// integrate: clamp steering, apply to velocity, clamp speed, move, wrap
...
main :: proc() {
	rl.InitWindow(SCREEN_W, SCREEN_H, "Boids 2/4 — naïve O(n²)")
	...
	for !rl.WindowShouldClose() {
		...
		// time ONLY the flock update — this is the number lesson 8.3 attacks
		t0 := rl.GetTime()
